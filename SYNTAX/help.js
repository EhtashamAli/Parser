...
bool PARSER::TERM(string expected){
    if(getNextToken().lexeme == expected){
        // cout << peekNextToken().lexeme << " ^^ " << expected << endl;
        return true;
    } else {
...
bool PARSER::TERM_TYPE(string expected){
    if(getNextToken().type == expected){
        // cout << peekNextToken().type << " ^^ " << expected << endl;
        return true;
    } else {
        LAST_EXPECTED_TYPE = expected;
        return false;
    }
    return getNextToken().type == expected;
}
...
bool PARSER::EXPRESSION(){
    return //(save_cursor() && TERM("(") && EXPRESSION() && TERM(")")) ||
            ...
            (save_cursor() && TERM_TYPE(IDENTIFIER) && TERM_TYPE(OPERATOR) && TERM_TYPE(IDENTIFIER)) ||
            (back_track() && save_cursor() && TERM_TYPE(OPERATOR) && TERM_TYPE(IDENTIFIER)) ||
            (back_track() && false);
...
TOKEN PARSER::peekNextToken()
{
    ...
    if(nextToken.type == "NEWLINE") {
        nextToken = this->TOKENS[CURSOR+1];
        return nextToken;
    }
    return nextToken;
}