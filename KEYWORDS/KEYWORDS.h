#ifndef KEYWORDS_H
#define KEYWORDS_H

#include <string>

const char PLUS_OP = '+';
const char MIN_OP = '-';
const char DIV_OP = '/';
const char MUL_OP = '*';
const char EQ_OP = '=';
const char NEQ_OP = '!';
const char LSS_OP = '<';
const char GTR_OP = '>';
const char MOD_OP = '%';
const char SEMI_COLON = ';';
const char L_BRACE = '{';
const char R_BRACE = '}';
const char L_PAREN = '(';
const char R_PAREN = ')';

const string KW_HEADER="header";
const string KW_USING="using";
const string KW_NAMESPACE="namespace";
const string KW_IOSTREAM="iostream";
const string KW_STD="std";
const string KW_CONTAINER="container";
const string KW_LOOP="loop";
const string KW_LOOPUNTIL="loopuntil";
const string KW_UNTIL="until";
const string KW_THEN="then";
const string KW_THIS="this";
const string KW_DIGIT="digit";
const string KW_DECIMAL="decimal";
const string KW_DOUBLE="double";
const string KW_CHOICE="choice";
const string KW_GLOBAL="global";
const string KW_OUTPUT="output";
const string KW_INPUT="input";
const string KW_RETURN="return";
const string KW_BOOLEAN="boolean";
const string KW_BYDEFAULT="bydefault";
const string KW_VACCANT="vaccant";
const string KW_EMPTY="empty";
const string KW_SELECT="select";

const string KEYWORD = "KEYWORD";
const string IDENTIFIER = "IDENTIFIER";
const string NUMBER = "NUMBER";
const string OPERATOR = "OPERATOR";
const string SYMBOL = "SYMBOL";
const string STRING = "STRING";
const string CHAR = "CHAR";

#endif